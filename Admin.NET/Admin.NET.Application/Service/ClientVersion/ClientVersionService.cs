...
public class ClientVersionService : IDynamicApiController, ITransient
{
    ...
    public async Task<ClientOssConfigOutput> GetOssConfig([FromQuery] ClientOssConfigQueryInput input)
    {
        ...
        var config = await FindOssConfigAsync(provider) ?? throw Oops.Oh($"未找到 {provider} 的 OSS 凭证，请先配置");
    ...
    public async Task<ClientOssConfigOutput> SaveOssConfig(ClientOssConfigInput input)
    {
        ...
        if (input.IsDefault)
        {
            await _ossConfigRep.AsUpdateable()
                ...
                .Where(u => u.Provider == provider && u.Id != entity.Id)
                .ExecuteCommandAsync();
        }
    ...
    public async Task<ClientVersionRuleOutput> SaveRule(ClientVersionRuleSaveInput input)
    {
        ...
        var rootPath = NormalizeRootPathForStorage(input.RootPath);
        var resourceDomain = NormalizeResourceDomain(input.ResourceDomain);
    ...
    {
        var context = await ResolveOssContextAsync(input.ChannelId, input.Platform, input.BucketName, input.RootPath);
        var client = CreateOssClient(context.Config);
    ...
    private async Task<ClientResourceOssConfig> GetActiveOssConfigAsync(string provider = "aliyun")
    {
        return await FindOssConfigAsync(provider) ?? throw Oops.Oh("尚未配置客户端资源 OSS 凭证");
    }
    ...
    private async Task ValidateOssConnectivityAsync(ClientResourceOssConfig config)
    {
        try
        {
            var client = CreateOssClient(config);
            var existed = await Task.Run(() => client.DoesBucketExist(config.BucketName));
            if (!existed)
            {
        ...
        {
            _logger.LogError(ex, "验证 OSS 凭证失败");
            throw Oops.Oh($"验证 OSS 凭证失败：{ex.Message}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "验证 OSS 凭证失败");
    ...
    private OssClient CreateOssClient(ClientResourceOssConfig config)
    {
        var secret = CryptogramUtil.Decrypt(config.AccessKeySecretCipher);
        return new OssClient(config.Endpoint, config.AccessKeyId, secret);
    }
    ...
    private async Task<OssContext> ResolveOssContextAsync(string channelId, ClientPlatform platform, string? bucketOverride, string? rootOverride)
    {
        var provider = "aliyun";
        var config = await GetActiveOssConfigAsync(provider);
        ...
        if (rule == null && string.IsNullOrWhiteSpace(bucketOverride))
            throw Oops.Oh("请先配置对应渠道的规则，或临时传入 Bucket / RootPath");

        bucketName = rule?.BucketName ?? bucketOverride!.Trim();
        rootPath = NormalizeRootPathForStorage(rule?.RootPath ?? rootOverride);
    ...
    private async Task<ClientAppVersionInfoDto?> NormalizeVersionInfoAsync(OssClient client,
    ...
    private async Task EnsureAppVersionExistsAsync(OssClient client, string bucket, string normalizedRoot, ClientPlatform platform, string appVersion)
    {
        var prefix = BuildPrefix(normalizedRoot, platform, appVersion);
        try
        {
            var result = await Task.Run(() => client.ListObjects(new ListObjectsRequest(bucket)
            {
                Prefix = prefix,
                MaxKeys = 1
            }));
            ...
            if (!result.ObjectSummaries.Any())
                throw Oops.Oh($"AppVersion {appVersion} 尚未上传任何资源，请检查 OSS 目录");
        }
        catch (OssException ex)
        {
    ...
    private async Task PopulateResVersionMetadataAsync(OssClient client,
        ClientResourceOssConfig config,
        ClientVersionRule rule,
        ClientAppVersionInfoDto info,
        string normalizedRoot)
    {
        ...
        try
        {
            var metadata = await Task.Run(() => client.GetObjectMetadata(rule.BucketName, key));
    ...
    private async Task<List<ChannelAppVersionItem>> ListAppVersionsAsync(OssClient client, OssContext context, ClientPlatform platform)
    {
        ...
        do
        {
            var request = new ListObjectsRequest(context.BucketName)
            {
            ...
            var result = await Task.Run(() => client.ListObjects(request));
            foreach (var commonPrefix in result.CommonPrefixes)
            {
                var folderName = ExtractFolderName(commonPrefix, request.Prefix);
    ...
    private async Task<List<ChannelResVersionItem>> ListResVersionsAsync(OssClient client, OssContext context, ClientPlatform platform, string appVersion)
    {
    ...
    {
        return string.IsNullOrWhiteSpace(provider) ? "aliyun" : provider.Trim().ToLowerInvariant();
    }
